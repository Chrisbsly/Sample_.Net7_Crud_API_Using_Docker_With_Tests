...
namespace CustomerTest.Infrastructure.Persistence.Repositories
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public async Task<List<Customer>> GetCustomersAsync(CancellationToken ct, int offset = 0, int limit = 10)
        {
        ...
        public async Task<bool> IsDuplicateEmailAsync(Guid id, string Email, CancellationToken ct)
        {
            return await _dbContext.Set<Customer>()
                .AnyAsync(x => x.Id != id && x.Email == Email, cancellationToken: ct);
        }


        public async Task<ErrorOr<EditCustomerResult>> EditCustomerAsync(EditCustomerCommand command,
